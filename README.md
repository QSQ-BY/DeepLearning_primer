...
书本练习使用 Python 和 PyTorch。运行 `source/1.预备知识/test_torch.py` 可以查看本机的 PyTorch 版本、CUDA 可用状态，并执行一段基础 Tensor 运算。`简单神经网络运算框架的实现` 只使用 NumPy 和 Python 标准库，单元测试使用 `unittest`；目前已经接通 Linear、ReLU、Conv2D、Flatten、Softmax 交叉熵、Sequential 和 SGD，并用合成图像跑通了离线小型 CNN。下一步将实现 BatchNorm。
